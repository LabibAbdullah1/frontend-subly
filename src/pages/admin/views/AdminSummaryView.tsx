  ...
  return (
    <div className="space-y-6 w-full text-left">
      ...
      {globalIssues.filter((i) => i.status !== 'resolved').length > 0 && (
        <div className="p-4 rounded-2xl bg-red-500/5 border border-red-500/10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 animate-in fade-in duration-300">
          <div className="flex items-center gap-2.5 text-xs font-bold text-red-500 text-left">
            ...
            <div>
              <h4 className="font-bold uppercase tracking-wider">{t('activeTicketAlertTitle')}</h4>
              <p className="text-[10px] text-text-muted font-semibold mt-0.5">
                {t('activeTicketAlertSub').replace('{count}', String(globalIssues.filter((i) => i.status !== 'resolved').length))}
              </p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 select-none">
        {[
          { label: t('totalClients'),         value: adminStats?.totalUsers ?? adminUsers.length,               icon: <Users className="h-5 w-5" /> },
          { label: t('deployment').toUpperCase(), value: `${activeQueueDeployments.length} ${t('queuePendingLabel').toUpperCase()}`, icon: <Repeat className="h-5 w-5" /> },
          { label: t('activeTickets'),        value: globalIssues.filter((i) => i.status !== 'resolved').length, icon: <MessageSquare className="h-5 w-5" /> },
          { label: t('monthlyRevenueLabel'),  value: `Rp ${monthlyRevenue.toLocaleString('id-ID')}`,            icon: <CreditCard className="h-5 w-5" /> },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <CardPanel className="p-6 flex flex-col items-center justify-center text-center">
            <h3 className="text-xs font-black uppercase text-text-muted tracking-wider mb-4">{t('globalServerStorage')}</h3>
            <div className="relative h-36 w-36 flex items-center justify-center">
              ...
              <div className="absolute flex flex-col items-center justify-center">
                ...
                <span className="text-[8px] font-black text-text-muted uppercase">{t('used')}</span>
              </div>
            </div>
          ...
          {/* Server Health */}
          <CardPanel title={t('serverResourcesTitle')}>
            <div className="space-y-4 text-xs font-semibold select-none mt-2">
              ...
              {[
                [
                  { label: t('cpuCores'),           value: adminStats?.system?.cpuCores ? `${adminStats.system.cpuCores} Cores` : '-' },
                  { label: t('nprocLimit'), value: adminStats?.system?.activeProcesses ? `${adminStats.system.activeProcesses} / ${adminStats.system.maxProcesses || 200} (${Math.round((adminStats.system.activeProcesses / (adminStats.system.maxProcesses || 200)) * 100)}%)` : '-', accent: true },
                ],
                [
                  { label: t('entryProcesses'), value: adminStats?.system ? `${adminStats.system.entryProcesses} / ${adminStats.system.maxEntryProcesses} (${Math.round((adminStats.system.entryProcesses / adminStats.system.maxEntryProcesses) * 100)}%)` : '-' },
                  { label: t('appUptime'),      value: adminStats?.system ? formatUptime(adminStats.system.uptimeSeconds) : '-' },
                ],
                [
                  { label: t('ioSpeed'), value: adminStats?.system ? `${adminStats.system.ioSpeedKb > 0 ? `${adminStats.system.ioSpeedKb} KB/s` : '0 bytes/s'} / ${adminStats.system.maxIoSpeedMb} MB/s` : '-' },
                  { label: t('iopsLimit'),      value: adminStats?.system ? `${adminStats.system.iops} / ${adminStats.system.maxIops.toLocaleString('id-ID')} (${Math.round((adminStats.system.iops / adminStats.system.maxIops) * 100)}%)` : '-' },
                ],
              ].map((row, ri) => (
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Recent Deployments */}
          <CardPanel title={t('recentDeployments')} headerActions={
            <button onClick={() => setActiveTab('admin-deployment')} className="text-[10px] font-bold text-brand-primary hover:underline flex items-center gap-1 cursor-pointer border-none bg-transparent">
              {t('viewQueue')} <ArrowRight className="h-3 w-3" />
            </button>
          }>
              ...
              <table className="w-full text-left min-w-[500px]">
                <thead>
                  <tr className="border-b border-border-main/50 text-[9px] text-text-muted uppercase tracking-widest">
                    ...
                    <th className="py-2.5 pb-2 px-4 text-right font-bold">{t('colStatus').toUpperCase()}</th>
                  </tr>
                </thead>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Revenue Bar Chart */}
            <CardPanel title={t('monthlyRevenueTrend')}>
            ...
            {/* Activity Log */}
            <CardPanel title={t('recentAuditLogs')}>
                  ...
                  return (
                    <div key={idx} className="flex items-start justify-between gap-3 text-[10px] border-b border-border-main/20 pb-2.5 last:border-0 last:pb-0 select-none">
                      ...
                      <span className="text-[9px] text-text-muted font-mono shrink-0">
                        {log.time.toLocaleTimeString(timeLocale, { hour: '2-digit', minute: '2-digit' })}
                      </span>
                    </div>
                  );
                })}
                {getSystemLogs().length === 0 && <p className="text-xs text-text-muted italic text-center py-6">{t('noSystemLogs')}</p>}
              </div>
            </CardPanel>
          </div>
        </div>
