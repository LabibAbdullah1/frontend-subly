...
export const AdminReportsView: React.FC = () => {
  ...
  const handleResolve = async (id: number) => {
    try {
      ...
      addToast({ type: 'success', title: 'Masalah Diselesaikan', message: 'Laporan tiket dukungan berhasil ditandai sebagai selesai.' });
    } catch {
      addToast({ type: 'error', title: 'Gagal', message: 'Gagal menyelesaikan masalah.' });
    }
  };
  ...
  return (
    <div className="space-y-6 w-full text-left">
      <div>
        <h1 className="text-xl md:text-2xl font-bold text-text-main tracking-tight uppercase">Laporan Tiket Dukungan</h1>
              ...
              {globalIssues.map((issue) => (
                <tr key={issue.id} className="hover:bg-border-main/5 transition-colors">
                  ...
                  <td className="py-3 px-4 text-center">
                    <Badge status={issue.status === 'resolved' ? 'success' : 'pending'} label={issue.status === 'resolved' ? 'SELESAI' : 'BUKA'} />
                  </td>
                  <td className="py-3 px-4 text-right">
                    {issue.status !== 'resolved' && (
                      <Button size="sm" variant="primary" onClick={() => handleResolve(issue.id)}>Selesaikan</Button>
                    )}
                  </td>
                </tr>
              ))}
              {globalIssues.length === 0 && (
                <tr><td colSpan={4} className="py-8 text-center text-text-muted italic">Tidak ada tiket dukungan masuk.</td></tr>
              )}
            </tbody>
          </table>
        </div>
