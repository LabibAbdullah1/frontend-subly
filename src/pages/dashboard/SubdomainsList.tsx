...
export const SubdomainsList: React.FC = () => {
  ...
  const { subdomains, deleteSubdomain, payments, addSubdomain } = useDataStore();
    ...
    <div className="space-y-6 w-full text-left">
      ...
      {unclaimedSlots.length > 0 && (
        <div className="space-y-3.5 pt-2">
          <h2 className="text-xs font-black text-brand-primary uppercase tracking-widest flex items-center gap-1.5 animate-pulse select-none">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {unclaimedSlots.map((slot) => (
              <CardPanel 
                key={slot.id} 
                className="border-2 border-brand-primary/30 bg-brand-primary/5 hover:border-brand-primary transition-all relative overflow-hidden"
                glow={true}
                glowColor="rgba(94, 106, 210, 0.15)"
              >
                <div className="absolute top-0 right-0 bg-brand-primary text-bg-base text-[8px] font-black uppercase px-2.5 py-1 rounded-bl-xl tracking-wider shadow-sm select-none">
                  Paid
                </div>
                <div className="space-y-4">
                  <div>
                    <span className="text-[9px] font-bold text-text-muted uppercase tracking-wider block select-none">Sewa Hosting Aktif</span>
                    <h3 className="text-sm font-black text-text-main mt-0.5">
      ...
      {subdomains.length > 0 && (
        <h2 className="text-xs font-black text-text-muted uppercase tracking-widest select-none pt-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {subdomains.map((sub) => (
          <CardPanel 
            key={sub.id} 
            className="flex flex-col justify-between h-full hover:-translate-y-1 transition-all"
            glow={true}
          >
            <div className="space-y-4">
              {/* Header card info */}
            ...
            <div className="flex gap-2.5 mt-5 pt-4 border-t border-border-main/40 select-none">
              ...
              <button
                ...
                className="text-text-muted hover:text-red-500 p-2.5 rounded-xl hover:bg-red-500/10 border border-transparent hover:border-red-500/10 cursor-pointer active:scale-95 transition-all shrink-0"
      ...
      >
        <div className="p-3.5 rounded-2xl bg-red-500/5 border border-red-500/10 text-red-500 flex items-center gap-2 select-none">
